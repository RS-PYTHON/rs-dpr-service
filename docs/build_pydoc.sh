...
MODULES_DIR="${PROJECT_ROOT_DIR}"
MKDOCS_FILE="${SCRIPT_DIR}/mkdocs.txt"
BASE_FOR_MKDOCS_LOCATIONS="rs-dpr-service/docs/doc/pydoc/"

# LIST OF MODULES IN THE PROJECT
declare -a MODULE_NAMES=("rs_dpr_service")
...
create_documentation_for_rs_dpr_service() {
    # Main function to generate the documentation of rs_client_libraries with all the specified modules
    #
    # USAGE:
    # create_documentation_for_rs_dpr_service
    ...
    for i in "${!MODULE_NAMES[@]}"; do
        ...
        create_documentation_for_module $module_name $MODULES_DIR/$module_name/

        echo "✓ Documentation for module $module_name located at $MODULES_DIR/$module_name/ succesfully built."
    done
}
